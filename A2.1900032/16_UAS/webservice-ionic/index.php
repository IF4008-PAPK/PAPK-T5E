...
function Input_Barang($request, $response)
{
    ...
    try {
        //if($systemToken == $token){
        $db = getDB();
        $sql = "INSERT INTO barang(nama_barang, terjual, jumlah, omzet) VALUES(:nama_barang ,:terjual, :jumlah, :omzet)";
        $stmt = $db->prepare($sql);
        $stmt->bindParam("nama_barang", $nama_barang, PDO::PARAM_STR);
        $stmt->bindParam("terjual", $terjual, PDO::PARAM_STR);
...
function Edit_Barang($request, $response)
{
    ...
    try {
        //if($systemToken == $token){
        $db = getDB();
        $sql = "UPDATE barang SET nama_barang=:nama_barang, terjual=:terjual, jumlah=:jumlah, omzet=:omzet WHERE id_barang=:id_barang";
        $stmt = $db->prepare($sql);
        $stmt->bindParam("id_barang", $id_barang, PDO::PARAM_STR);
        $stmt->bindParam("nama_barang", $nama_barang, PDO::PARAM_STR);
        $stmt->bindParam("terjual", $terjual, PDO::PARAM_STR);
