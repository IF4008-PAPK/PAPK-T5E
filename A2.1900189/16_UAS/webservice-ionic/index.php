...
function Input_Barang($request, $response)
{
    ...
    try {
        //if($systemToken == $token){
        $db = getDB();
        $sql = "INSERT INTO barang(nama_lengkap, nik, alamat) VALUES(:nama_lengkap ,:nik, :alamat)";
        $stmt = $db->prepare($sql);
...
function Edit_Barang($request, $response)
{
    ...
    try {
        //if($systemToken == $token){
        $db = getDB();
        $sql = "UPDATE barang SET nama_lengkap=:nama_lengkap, nik=:nik, alamat=:alamat WHERE id_barang=:id_barang";
        $stmt = $db->prepare($sql);
        $stmt->bindParam("id_barang", $id_barang, PDO::PARAM_STR);
        $stmt->bindParam("nama_lengkap", $nama_lengkap, PDO::PARAM_STR);
        $stmt->bindParam("nik", $nik, PDO::PARAM_STR);
